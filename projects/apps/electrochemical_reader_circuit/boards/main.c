...
uint32_t get_monotonic_now(void)
{
...
void bluetooth_to_main_commands_runner(void)
{
	while (true)
	{
		...
		main_commands_ctx.main_commands_buffer_ctx = &(MAIN_COMMANDS_buffer_ctx) {
			.buffer = data,
			.len = len,
		};
...
static volatile atomic_uint_fast32_t bluetooth_send_ad5940_adc_fifo_runner_deadline = 1000;

void bluetooth_send_ad5940_adc_fifo_runner(void)
{
	while (true)
	{
		atomic_store(&bluetooth_send_ad5940_adc_fifo_runner_deadline, k_uptime_get_32() + 1000);
...
void watch_dog_runner(void)
{
	while (true)
	{
		...
		{
			uint32_t deadline = atomic_load(main_commands_ctx.main_commands_state_ctx->deadline);
			uint32_t difference = (uint32_t)(now - deadline);
			if (difference > WDG_TIMEOUT_TICKS && difference < WDG_MAX_TICKS) break;
		}

		if (atomic_load(vice_commands_ctx.vice_commands_state_ctx->is_working))
		{
			uint32_t deadline = atomic_load(vice_commands_ctx.vice_commands_state_ctx->deadline);
			uint32_t difference = (uint32_t)(now - deadline);
			if (difference > WDG_TIMEOUT_TICKS && difference < WDG_MAX_TICKS) break;
		}

		{
			uint32_t deadline = atomic_load(&bluetooth_send_ad5940_adc_fifo_runner_deadline);
			uint32_t difference = (uint32_t)(now - deadline);
			if (difference > WDG_TIMEOUT_TICKS && difference < WDG_MAX_TICKS) break;
		}
...
// TODO
// Test AD5940 Task
uint8_t buffer[] = {, };
...
int main(void)
{
	...
	vice_commands_ctx = (VICE_COMMANDS_ctx) {
		...
		.delay_time_to_ms = COMMANDS_DELAY_TIMEOUT.ticks / (CONFIG_SYS_CLOCK_TICKS_PER_SEC / 1E3),
		.free = k_free,
