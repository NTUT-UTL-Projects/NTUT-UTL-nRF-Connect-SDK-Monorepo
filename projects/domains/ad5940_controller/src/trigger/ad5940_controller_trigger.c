...
AD5940Err AD5940_controller_trigger(AD5940_CONTROLLER_TRIGGER_PARA *const para)
{
    // ========================================
     ...
     * Therefore, they should not be used during the configuration process
     * itself.
     */
    AD5940_GPIOClr();
    ...
    SEQCfg_Type seq_cfg = {
        ...
        .SeqCntCRCClr = bTRUE,
        .SeqEnable =
            bFALSE, // Refer to page 93 of the datasheet, it stop the SEQ.
        .SeqIgnoreEn = bTRUE,
    ...
    FIFOCfg_Type fifo_cfg = {
        .FIFOEn = bFALSE, // Refer to page 94 of the datasheet, it reset the
                          // FIFO pointer.
        .FIFOMode = FIFOMODE_FIFO, // Refer to page 94 of the datasheet
        .FIFOSize = FIFOSIZE_2KB,  // Refer to page 94 of the datasheet,
                                  // The strategy is to read the FIFO when there
                                  // is data in the FIFO, so there is no need to
                                  // keep a large FIFOSIZE.
...
#define SEQLEN_ONESTEP                                                         \
    ((para->EnterSleepEn == bTRUE)                                             \
         ? 4L                                                                  \
         : 3L) /* How many sequence commands are needed to update LPDAC. */
            ...
            ELECTROCHEMICAL_DEV_KIT_NUMBER_TYPE n_step_total =
                ELECTROCHEMICAL_DEV_KIT_cv_get_n_step_total(
                    &para->event.param.cv.cv
                );
            for (size_t i = 0; i < n_step_total; i++)
            {
                e_current = ELECTROCHEMICAL_DEV_KIT_cv_get_e_at_index(
                    &para->event.param.cv.cv,
                    i
                );
                lpdac_dat_6bit = AD5940_volts_to_LPDACData6Bit(e_current);
                lpdac_dat_12bit =
                    AD5940_volts_to_LPDACData12Bit(e_current, lpdac_dat_6bit);
                ...
                SeqCmdBuff[1] = SEQ_WAIT(
                    10
                ); /* !!!NOTE LPDAC need 10 clocks to update data. Before send
                      AFE to sleep state, wait 10 extra clocks */
                BoolFlag is_final_step =
                    (i == (n_step_total - 1)) ? bTRUE : bFALSE;
                AD5940_SEQ_WR_SEQID(
                    (now_is_0) ? CV_1_SEQID : CV_0_SEQID,
                    is_final_step ? SeqRamAddr
                                  : (current_address + SEQLEN_ONESTEP),
                    SEQLEN_ONESTEP,
                    SeqCmdBuff + 2
                );
                if (para->EnterSleepEn)
            ...
            AD5940_WriteSEQID(CV_0_SEQID, SeqRamAddr, SEQLEN_ONESTEP);
            AD5940_WriteSEQID(
                CV_1_SEQID,
...
#define SEQLEN_ONESTEP                                                         \
         ...
         : 3L) /* How many sequence commands are needed to update LPDAC. */
            ...
            ELECTROCHEMICAL_DEV_KIT_NUMBER_TYPE n_step_total =
                ELECTROCHEMICAL_DEV_KIT_dpv_get_n_step_total(
                    &para->event.param.dpv
                );
            for (size_t i = 0; i < n_step_total; i++)
            {
                e_current = ELECTROCHEMICAL_DEV_KIT_dpv_get_e_at_index(
                    &para->event.param.dpv,
                    i,
            ...
            SeqLen = current_address - SeqRamAddr;
            AD5940_WriteSEQID(DPV_STEP_SEQID, SeqRamAddr, SEQLEN_ONESTEP);
            AD5940_WriteSEQID(
                DPV_PULSE_SEQID,
                SeqRamAddr + SEQLEN_ONESTEP,
                SEQLEN_ONESTEP
    ...
    AD5940_AFEINTCxCfg(AFEINTC_0_En, AFEINTC_1_En, AFEINTSRC_DATAFIFOTHRESH);

    AD5940_AGPIOCfg(&(para->gpio_cfg));

    /* Configure FIFO and Sequencer for normal Amperometric Measurement */
    uint32_t FifoThresh = 1; // Set to 1, so that when there is data in FIFO,
                             // GPIO will be triggered.
    ...
    switch (para->event.event)
    {
        case AD5940_CONTROLLER_EVENT_CA:
        {
            ...
            wupt_cfg.SeqxSleepTime[ADC_SEQID] =
                1; /* The minimum value is 1. Do not set it to zero. Set it to 1
                      will spend 2 32kHz clock. */
            wupt_cfg.SeqxWakeupTime[ADC_SEQID] =
                (uint32_t)(para->LFOSCClkFreq *
                           para->event.param.ca.t_interval) -
                1;
...
#define SAMPLE_DELAY 0.001f
            ...
            wupt_cfg.SeqxSleepTime[ADC_SEQID] =
                1; // The minimum value is 1. Do not set it to zero. Set it to 1
                   // will spend 2 32kHz clock.
            wupt_cfg.SeqxWakeupTime[ADC_SEQID] =
                (uint32_t)(para->LFOSCClkFreq * SAMPLE_DELAY) - 1;
            wupt_cfg.SeqxSleepTime[CV_0_SEQID] =
                1; // The minimum value is 1. Do not set it to zero. Set it to 1
                   // will spend 2 32kHz clock.
            wupt_cfg.SeqxWakeupTime[CV_0_SEQID] =
                (uint32_t)(para->LFOSCClkFreq *
                           (para->event.param.cv.cv.t_step - SAMPLE_DELAY)) -
                1;
            wupt_cfg.SeqxSleepTime[CV_1_SEQID] =
                1; // The minimum value is 1. Do not set it to zero. Set it to 1
                   // will spend 2 32kHz clock.
            wupt_cfg.SeqxWakeupTime[CV_1_SEQID] =
                (uint32_t)(para->LFOSCClkFreq *
                           (para->event.param.cv.cv.t_step - SAMPLE_DELAY)) -
                1;
#undef SAMPLE_DELAY
        ...
        case AD5940_CONTROLLER_EVENT_DPV:
        {
            ...
            t_interval = ELECTROCHEMICAL_DEV_KIT_dpv_get_t_interval(
                &para->event.param.dpv
            );
...
#define SAMPLE_DELAY 0.001f
            ...
            wupt_cfg.SeqxWakeupTime[ADC_SEQID] =
                (uint32_t)(para->LFOSCClkFreq * SAMPLE_DELAY) - 1;
            wupt_cfg.SeqxSleepTime[DPV_STEP_SEQID] =
                1; // The minimum value is 1. Do not set it to zero. Set it to 1
                   // will spend 2 32kHz clock.
            wupt_cfg.SeqxWakeupTime[DPV_STEP_SEQID] =
                (uint32_t)(para->LFOSCClkFreq *
                           (para->event.param.dpv.t_pulse - SAMPLE_DELAY)) -
                1;
            wupt_cfg.SeqxSleepTime[DPV_PULSE_SEQID] =
                1; // The minimum value is 1. Do not set it to zero. Set it to 1
                   // will spend 2 32kHz clock.
            wupt_cfg.SeqxWakeupTime[DPV_PULSE_SEQID] =
                (uint32_t)(para->LFOSCClkFreq *
                           (t_interval - para->event.param.dpv.t_pulse -
                            SAMPLE_DELAY)) -
                1;
#undef SAMPLE_DELAY
        ...
        case AD5940_CONTROLLER_EVENT_TEMPERATURE:
        {
            ...
            wupt_cfg.SeqxWakeupTime[ADC_SEQID] =
                (uint32_t)(para->LFOSCClkFreq *
                           para->event.param.temperature.t_interval) -
                1;
            AD5940_WUPTCfg(&wupt_cfg);
            break;
        }
