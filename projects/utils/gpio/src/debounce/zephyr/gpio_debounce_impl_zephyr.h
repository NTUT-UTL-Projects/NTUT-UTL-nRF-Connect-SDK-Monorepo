...
#include <zephyr/kernel.h>
     ...
     *
     * **Users do not need to manually initialize or modify the members
     * inside.** All necessary fields will be automatically configured by
     * `z_impl_gpio_debounce_init()`.
     */
    typedef struct
    {
        const struct gpio_dt_spec
            *gpio_dt; /**< Pointer to GPIO device tree specification. */
        struct gpio_callback
            gpio_cb; /**< GPIO callback structure for interrupt handling. */
        gpio_flags_t gpio_extra_flags; /**< Additional GPIO flags (e.g.,
                                          pull-up/down). */
        struct k_work_delayable
            debounce_work; /**< Zephyr delayable work structure for debounce
                              logic. */
        void (*rising_edge_callback)(
            void
        ); /**< Callback function to execute on rising edge detection. */
        void (*falling_edge_callback)(
            void
        ); /**< Callback function to execute on falling edge detection. */
        bool last_state; /**< Last recorded GPIO state, used for edge detection.
                          */
        k_timeout_t
            debounce_delay_ms; /**< Debounce delay time (in milliseconds). */
    } gpio_debounce_ctx_t;
     ...
     *
     * **Important:**
     * - Users only need to provide a pointer to an uninitialized
     * `gpio_debounce_ctx_t` structure.
     * - The function `z_impl_gpio_debounce_init()` will fully configure all
     * necessary members of `ctx`.
     * - **Do not manually modify the `ctx` fields.**
     *
     * @param ctx                   Pointer to an uninitialized debounce context
     * structure.
     ...
     * @param rising_edge_callback  Function to call on rising edge (can be
     * NULL).
     * @param falling_edge_callback Function to call on falling edge (can be
     * NULL).
     *
