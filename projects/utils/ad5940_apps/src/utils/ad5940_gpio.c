...
{
    if (((FuncSet & BITM_AGPIO_GP0CON_PIN0CFG) == GP0_INT) ||
        ((FuncSet & BITM_AGPIO_GP0CON_PIN3CFG) == GP3_INT0) ||
        ((FuncSet & BITM_AGPIO_GP0CON_PIN6CFG) == GP6_INT0))
        *AFEINTC_0_En = bTRUE;
    else
        *AFEINTC_0_En = bFALSE;
    if (((FuncSet & BITM_AGPIO_GP0CON_PIN4CFG) == GP4_INT1) ||
...
{
    if (AFEINTC_0_En)
        AD5940_INTCCfg(AFEINTC_0, AFEIntSrc, bTRUE);
    if (AFEINTC_1_En)
        AD5940_INTCCfg(AFEINTC_1, AFEIntSrc, bTRUE);
    return;
}
