 ...
 *
 * @note
 * Refer to page 30 of the datasheet for detailed information on setting the
 * voltage between V_bias and V_zero. The default MUX settings used in this file
 * are:
 * - `AD5940_LPDACVBIAS_12BIT` for V_bias (higher precision: 12 bits)
 * - `AD5940_LPDACVZERO_6BIT` for V_zero (lower precision: 6 bits)
 *
 * Since V_bias has higher precision than V_zero, the desired output voltage
 * (V_out) should be set as follows:
...
#define DAC6BITVOLT_1LSB  (64.0 * DAC12BITVOLT_1LSB) // V
    ...
    inline BoolFlag AD5940_LPDACData12Bit_is_greater(
    ...
    AD5940_volts_to_LPDACData12Bit(float volts, uint16_t DacData6Bit)
    {
        ...
        return LPDACData12Bit +
               (AD5940_LPDACData12Bit_is_greater(LPDACData12Bit, DacData6Bit)
                    ? 1
                    : 0);
    }

    inline uint32_t AD5940_LPDACDataBits_to_LPDACDAT(
        uint16_t DacData12Bit,
        uint16_t DacData6Bit
    )
    {
    ...
    inline float AD5940_DacData12Bit_to_volts(
        uint16_t DacData12Bit,
        BoolFlag LPDACData12Bit_is_greater
    )
    {
        return DacData12Bit * DAC12BITVOLT_1LSB +
               (LPDACData12Bit_is_greater ? -1 : 0);
    }
