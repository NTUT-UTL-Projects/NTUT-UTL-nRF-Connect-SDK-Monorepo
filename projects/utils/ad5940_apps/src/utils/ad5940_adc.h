...
inline float AD5940_fFreqCal(const AD5940_ADCIMPCheckFreq_Results *const results, uint32_t SamplePeriod)
{
    return (results->AdcClkFreq / AD5940_DFTNUM_to_val_table[results->freq_params.DftNum] / AD5940_ADCSINC2OSR_to_val_table[results->freq_params.ADCSinc2Osr] / AD5940_ADCSINC3OSR_to_val_table[results->freq_params.ADCSinc3Osr] * SamplePeriod);
}
...
inline float AD5940_ADCCode_to_volts(uint32_t ADCcode, uint32_t ADCPga, float VRef1p82)
{
    return (ADCPga == ADCPGA_1P5 ? 1.835 : VRef1p82) / AD5940_ADCPGA_to_val_table[ADCPga] * ADCcode / 0x8000;
}
