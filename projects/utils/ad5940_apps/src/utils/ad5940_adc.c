 ...
 */
const uint32_t AD5940_DFTNUM_to_val_table[] =
    {4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384};

/**
 * @brief Retrieves the ADC SINC2 oversampling rate based on the given constant.
 *
 * @param ADCSinc2Osr The ADC SINC2 oversampling rate constant. @see
 * ADCSINC2OSR_Const.
 */
const uint32_t AD5940_ADCSINC2OSR_to_val_table[] =
    {1, 22, 44, 89, 178, 267, 533, 640, 667, 800, 889, 1067, 1333};

/**
 * @brief Retrieves the ADC SINC3 oversampling rate based on the given constant.
 *
 * @param ADCSinc3Osr The ADC SINC3 oversampling rate constant. @see
 * ADCSINC3OSR_Const.
 */
...
/**
 * Calculates the calibration frequency used in AD5940_LPRtiaCal and
 * AD5940_HSRtiaCal.
 *
 ...
 */
extern inline float AD5940_fFreqCal(
 ...
 *                - Figure 34. Postprocessing Filter Options: Shows the option
 * should be raw ADC code (DATAFIFO_SINC3 or DATAFIFO_SINC2).
 *                - `DATAFIFOSRCSEL`: shows how to selects the source for the
 * data FIFO.
 *
 * @note The `ad5940.h` file provides an `AD5940_ReadAfeResult` function for
 * reading various DATAFIFOSRC.
 */
 ...
 * @param VRef1p82 Reference voltage for the ADC (in volts). Refer to datasheet
 * page 87.
 *
 * @note
 ...
 * @brief Converts ADC data to current values.
 *
 * @param RtiaCalValue Pointer to the RTIA calibration result. Available after
 * calibration:
 ...
 */
extern inline float
AD5940_volts_to_amperes(float volts, fImpPol_Type RtiaCalValue);
