...
void AD5940_LPDAC2LPTIA_AFERefCfg(AFERefCfg_Type *const type)
{
    ...
    // Refer to page 34 of the datasheet. See also the routing diagrams in
    // Figure 22 (page 39) and Figure 37 (page 87). This output is used to drive
    // the LPDAC, or to generate a bias voltage via the LPDAC when using the
    // HSDAC.
...
#endif
    type->DataRst =
        bFALSE; // Do not reset data register. @see p. 33 of the datasheet.
...
)
{
    ...
    {
        type->LpTiaSW = 0 | LPTIASW(2) | LPTIASW(4) | LPTIASW(5) | LPTIASW(9)
            // | LPTIASW(12)
            // | LPTIASW(13)
            ;
    }
    ...
    {
        type->LpTiaSW = 0 | LPTIASW(2) | LPTIASW(4)
            // | LPTIASW(5)
            // | LPTIASW(12)
            // | LPTIASW(13)
            ;
