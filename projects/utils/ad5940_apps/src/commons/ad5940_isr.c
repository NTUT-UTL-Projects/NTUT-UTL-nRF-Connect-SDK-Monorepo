...
AD5940Err
AD5940_DATAFIFOISR(AD5940_DATAFIFOISR_PARA para, AD5940_DATAFIFOISR_RET ret)
{
    ...
    AD5940_SleepKeyCtrlS(
        SLPKEY_LOCK
    ); /* We need time to read data from FIFO, so, do not let AD5940 goes to
          hibernate automatically */

    *(ret.pBuffCount) = AD5940_FIFOGetCnt();
    if (*(ret.pBuffCount) > para.BufferSize)
    ...
    else
    {
        ...
        /* For detailed reference on the chip's shutdown procedure, see page 129
         * of the datasheet.*/
        AD5940_ShutDownS();
    }
