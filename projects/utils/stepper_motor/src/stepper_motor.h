...
#include <stdint.h>
    ...
    typedef struct
    {
        ...
        uint8_t
            *sequence; // Flat array, length = seq_len * coil_count, values 0/1

        stepper_drive_mode_t mode;       // Drive mode
        gpio_write_fn        gpio_write; // GPIO control function
        delay_us_fn          delay_us;   // Delay function

        int base_steps_per_rev; // Mechanical “full-step” steps per revolution
                                // (based on FULL/WAVE)
        int effective_steps_per_rev; // Actual steps per revolution depending on
                                     // mode (half-step doubles it)
    ...
    void
    StepperMotor_RotateToAngle(StepperMotor *stp, float degree, int delay_us);
