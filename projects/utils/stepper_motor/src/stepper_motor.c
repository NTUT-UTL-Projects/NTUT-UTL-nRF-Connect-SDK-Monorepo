...
#define SEQ_AT(stp, step_idx, coil_idx)                                        \
    ((stp)->sequence[(step_idx) * (stp)->coil_count + (coil_idx)])
...
static bool generate_sequence(StepperMotor *stp)
{
    ...
    switch (stp->mode)
    {
        case STEPPER_WAVE_DRIVE:
            stp->seq_len = N;
            break; // One phase on per step
        case STEPPER_FULL_STEP:
            stp->seq_len = N;
            break; // Two adjacent phases on per step
        case STEPPER_HALF_STEP:
            stp->seq_len = 2 * N;
            break; // Alternate single/two-phase
        default:
            return false;
    }
...
)
{
    ...
    // In half-step mode, steps per revolution are doubled; otherwise same as
    // base
    stp->effective_steps_per_rev = (mode == STEPPER_HALF_STEP)
                                       ? (base_steps_per_rev * 2)
                                       : base_steps_per_rev;
